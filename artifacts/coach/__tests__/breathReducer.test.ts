...
describe("buildPhases", () => {
  ...
  it("assigns correct scale values per phase", () => {
    const phases = buildPhases({ inhale: 4, holdIn: 4, exhale: 4, holdOut: 4 });
    expect(phases.find((p) => p.key === "inhale")?.scale).toBe(1);
    expect(phases.find((p) => p.key === "holdIn")?.scale).toBe(1);
    expect(phases.find((p) => p.key === "exhale")?.scale).toBe(0.62);
    expect(phases.find((p) => p.key === "holdOut")?.scale).toBe(0.62);
  });
});
