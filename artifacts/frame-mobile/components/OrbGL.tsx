...
import { MeshDistortMaterial, Sparkles } from "@react-three/drei/native";
import * as THREE from "three";

/**
 * OrbGL — the REAL WebGL CosmicOrb from the website (artifacts/coach
 * cosmic-orb.tsx) rendered natively on the phone's GPU via expo-gl +
 * react-three-fiber. No WebView, no network: the identical Three.js scene the
 * site runs, drawn directly by the device. Drop-in replacement for OrbNative /
 * OrbWebView (same `state` + `size` props).
 */

// Mobile-side capitalized states, matching the labels used across the app
// (see app/(tabs)/state.tsx). "Streaming" has no mobile equivalent but is kept
// in VISUALS so an unexpected value never yields a blank frame.
type OrbState =
  | "Dormant"
  | "Stable"
  | "Loaded"
...
const VISUALS: Record<CfgKey, Cfg> = {
...
function toKey(state: OrbState): CfgKey {
  const k = state.toLowerCase() as CfgKey;
  return VISUALS[k] ? k : "stable";
}
...
/**
 * Two layered geodesic wireframes: a dense fine triangulation and a bolder
 * structural shell that drifts the other way — depth + a sense of energy flow.
 */
...
// multipliers (all scaled by the live ringSpeed).
interface RingDef {
  r: number;
  tube: number;
...
// The sphere spins +y (and slightly +x), so rings are given dominant -y / -x /
...
function SparkleField({ state }: { state: CfgKey }) {
  const cfg = VISUALS[state];
  const sparkleColor = useMemo(
    () => hsl(cfg.hue, cfg.sat, cfg.light + 0.05),
...
function Scene({ state }: { state: CfgKey }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-4, -2, -3]} intensity={0.1} color="#1a1a1a" />
...
> {
  ...
  componentDidCatch(_error: Error, _info: ErrorInfo) {
    // Swallow R3F/GL init failures — render nothing rather than crash the tab.
  }
  render() {
...
/**
 * The real cosmic orb, GPU-rendered natively. Same props as OrbNative/OrbWebView.
 */
export function OrbGL({ state = "Dormant", size = 200 }: Props) {
