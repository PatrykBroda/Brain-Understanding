...
- **Never combine free-text numeric inputs across incompatible units.** Weight fields are free text ("77kg", "170 lbs", "77"). The Camp weight-cut only subtracts when both units match (or both are absent); a mixed pair (lbs vs kg) must return the honest status "Calibrating", never an invented difference. Same principle for any future free-text numeric math: if you can't prove the units line up, don't produce a number.
